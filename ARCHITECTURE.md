...
`feature/custom-api-urls@e3ed1d6`。T002–T021 的基础设施与第一轮 Mock 已落地，但真实
产品账号、积分结算和 Provider 接入仍属于后续阶段。

```mermaid
flowchart LR
    Code[Code / AI Editor] -->|本机 nonce| Edge[Edge :47921]
    Edge -->|后续真实 Token 转发| Gateway[Gateway :47920]
...
| `src/edge/` | 本机 nonce、一次性 handoff、防重放和第一轮 Edge Mock |
| `gateway/src/app.ts` | Fastify Gateway、中间件与第一轮账号/模型 Mock |
