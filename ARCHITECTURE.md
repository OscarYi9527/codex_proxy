...
T060–T067 组织/RBAC 后端和 T081–T089 Provider 管理已经落地；组织 React 页面、
积分风险和结算仍属于 T068–T080 后续阶段。
...
- 组织和邀请码 React 页面仍为占位；服务端已完成显式 organization scope、跨组织
  `403`、Level-2 普通用户/邀请码边界、旧会话失效和最后 Level-1 事务保护，N016
  全管理动作审计闭环与正文保留仍未完成。
- Gateway Provider 新凭据统一使用 `envelope-v1`，并已提供密钥轮换和明文迁移；
  N008 全链路 Secret Scan 和发布门禁已通过；真实 production 部署仍须使用独立数据根、
  HTTPS、受保护主密钥并完成部署侧脱敏验收。
...
RK3588 使用独立 `rk3588` 模式，不复用 standalone 账号池，也不在设备上保存最终
Codex/OpenAI Key。生产流量拓扑为：

```mermaid
flowchart LR
    C[同事 Codex] -->|tailnet HTTPS + client key| T[Tailscale Serve]
    T -->|loopback :47930| R[RK3588 relay]
    R -->|HTTPS + RK→JP key| J[日本 relay]
    J -->|独立上游认证| O[Codex / Responses server]
```

RK 进程强制绑定 `127.0.0.1`，Tailscale ACL 是网络身份门禁，应用 Bearer Key 是第二层
授权。入站 Authorization 在 RK 终止，转发时替换为日本节点专用 Key；日本节点再替换为
最终服务 Key，因此任何单跳凭据都不能越级访问下一层。配置只从权限受限文件读取，
客户端 Key 在内存配置中只保留摘要，上游 Key 属性不可枚举且不会进入响应或日志。

relay 只允许模型目录、Responses 和 Chat Completions 三个 API 路径，限制 Host、正文、
超时与在途并发，白名单转发响应头并保持 SSE 流式。POST 在中转层不重试，避免断线或
超时造成重复 Turn 和重复计费。容器使用 ARM64 可用的官方 Node 22 Debian 镜像；
RK 模式只依赖 Node 内置模块，无需编译 Gateway 的 SQLite 原生模块。完整部署和验收见
...
| `src/launcher.js` / `src/mode.js` | standalone 默认入口及 Edge、Gateway、RK3588 模式选择 |
| `src/edge/` | 本机 nonce、OS 安全存储、一次性 handoff、账号绑定和真实/Mock 转发 |
| `src/rk3588/` | tailnet 私域入口、文件 Key 鉴权、受限并发及日本 HTTPS 流式中转 |
...
| `gateway/src/organizations/` | Level-1/Level-2 集中授权、组织/账号生命周期和最后管理员保护 |
| `gateway/src/invitations/` | 组织绑定邀请码的一次性签发、摘要存储、撤销和并发消费 |
