function renderAccounts(){
  ...
  const compactRows=accounts.map((a,index)=>{
    ...
    return `<article class="account-compact-row ${routeTone==='off'?'is-muted':''}" style="--account-hue:${hue}" draggable="true" data-account-id="${esc(a.id)}" ondragstart="startAccountDrag(event,'${esc(a.id)}')" ondragover="event.preventDefault()" ondrop="dropAccount(event,'${esc(a.id)}')">
      ...
      <div class="compact-actions">
        ...
        <button title="同步额度和重置次数" onclick="refreshAccountUsageOne('${esc(a.id)}')">${svg('refresh')}</button>
  ...
  const actions=button('官方安全登录','shield','openOfficialLogin(event)','btn-primary')+button('批量登录队列','users','openBatchOfficialLogin(event)')+button('快捷/批量导入','plus','openAccount()')+button('检查所有账号','pulse','checkAllAccountStatus()','btn-primary')+button('同步全部额度/次数','refresh','refreshAllUsage()')+button('查询全部重置次数','pulse','refreshAllResetCredits()')+button('重启 Codex','refresh','restartCodex()')
  ...
  return pageHead('ChatGPT 账号池','以账号为中心查看额度、性能、调度和重置能力',actions)+`<div class="metrics">${metric('账号总数',allAccounts.length,'users',`稳定 ${categoryCounts.stable_pool} · 日抛 ${categoryCounts.disposable_pool} · 已弃 ${categoryCounts.discarded}`)}${metric('有效账号',available,'check',`${allAccounts.length-available} 个账号当前不可调度`)}${metric('24h 内到期',categoryCounts.expiring,'pulse',`已失效 ${categoryCounts.expired} · 不兼容 ${categoryCounts.incompatible}`)}${metric('当前本机账号',activeLabel?esc(activeLabel.label||activeLabel.account_id||'已选择'):'未选择','shield',`等待队列 ${Number(diagnosticsData.queue?.depth)||0}`)}</div>`+strategyBody+accountBoard+card('最近路由决策','解释每次请求为什么选择或跳过某个账号',decisionBody)
}
