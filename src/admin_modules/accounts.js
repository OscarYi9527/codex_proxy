function renderAccounts(){
  ...
  const available=allAccounts.filter(a=>(a.status==='active'||!a.status)&&a.routing_enabled!==false&&credentialOf(a).compatible&&!credentialOf(a).expired&&(remainingOf(a)==null||remainingOf(a)>threshold)).length
  ...
  const cards=accounts.map((a,index)=>{
    ...
    const routeLabel=!credential.compatible?'凭据不兼容':credential.expired?'令牌到期':!routeEnabled?'仅保存':a.status==='cooldown'?'冷却中':a.status==='auth_error'?'登录失效':atReserve?'额度保护':'参与路由'
    const routeTone=!credential.compatible||credential.expired||!routeEnabled||a.status==='auth_error'?'off':a.status==='cooldown'||atReserve||credential.expiringSoon?'warn':'ok'
  ...
  const compactRows=accounts.map((a,index)=>{
    ...
    return `<article class="account-compact-row ${routeTone==='off'?'is-muted':''}" style="--account-hue:${hue}" draggable="true" data-account-id="${esc(a.id)}" ondragstart="startAccountDrag(event,'${esc(a.id)}')" ondragover="event.preventDefault()" ondrop="dropAccount(event,'${esc(a.id)}')">
      <div class="compact-identity">
        <span class="account-rank drag-handle" title="拖拽调整优先级">${globalIndex+1}</span>
        <div class="account-avatar">${esc(initials)}</div>
        <div><strong>${esc(label)}</strong><small>${!credential.compatible?'OAuth 不兼容':credential.temporary?`临时 ${esc(credential.countdown)}`:'可续约'} · ${esc(a.plan_type||'套餐待同步')} · ${esc((a.account_id||a.id||'-').slice(0,16))}</small></div>
      </div>
      ...
      <div class="compact-status">
        ...
        <small>${credential.temporary?`到期 <b>${esc(credential.countdown)}</b> · `:''}重置次数 <b>${resetCount==null?'待查询':resetCount}</b>${resetCount>0?' · <em class="reset-risk-label" style="color:var(--red);font-style:normal;font-weight:800">高风险</em>':''}${isActive?' · 本机账号':''}</small>
      </div>
      <div class="compact-actions">
        ${!isActive&&!credential.temporary?`<button title="切换为本机账号" onclick="switchAccount('${esc(a.id)}')">${svg('arrow')}</button>`:''}
  ...
  const categorySwitch=`<div class="account-view-switch" style="flex-wrap:wrap" role="group" aria-label="账号凭据分类">${[['all','全部'],['refreshable','可续约'],['temporary','临时'],['expiring','24h内到期'],['expired','已失效'],['incompatible','不兼容']].map(([value,label])=>`<button class="${accountCategory===value?'active':''}" onclick="setAccountCategory('${value}')"><span>${label} ${categoryCounts[value]}</span></button>`).join('')}</div>`
  ...
  const actions=button('官方安全登录','shield','openOfficialLogin()','btn-primary')+button('批量登录队列','users','openBatchOfficialLogin()')+button('快捷/批量导入','plus','openAccount()')+button('刷新全部用量','refresh','refreshAllUsage()')+button('查询全部重置次数','pulse','refreshAllResetCredits()')+button('重启 Codex','refresh','restartCodex()')
  ...
  return pageHead('ChatGPT 账号池','以账号为中心查看额度、性能、调度和重置能力',actions)+`<div class="metrics">${metric('账号总数',allAccounts.length,'users',`临时 ${categoryCounts.temporary} · 可续约 ${categoryCounts.refreshable} · 不兼容 ${categoryCounts.incompatible}`)}${metric('有效账号',available,'check',`${allAccounts.length-available} 个账号当前不可调度`)}${metric('24h 内到期',categoryCounts.expiring,'pulse',`已失效 ${categoryCounts.expired} 个`)}${metric('当前本机账号',activeLabel?esc(activeLabel.label||activeLabel.account_id||'已选择'):'未选择','shield',`等待队列 ${Number(diagnosticsData.queue?.depth)||0}`)}</div>`+strategyBody+accountBoard+card('最近路由决策','解释每次请求为什么选择或跳过某个账号',decisionBody)
}
