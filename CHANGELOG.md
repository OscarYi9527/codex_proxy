...
- 账号检查的单请求超时改用可清理且保持事件循环存活的定时器；发布门禁在无 Provider
  Key 的隔离环境中也按真实 readiness 语义断言，消除 Windows CI 的悬空 Promise 和
  环境依赖。
- 更新脚本使用 .NET SHA-256 API 计算文件摘要，不再依赖部分精简 PowerShell 环境缺失的
  `Get-FileHash` cmdlet。
