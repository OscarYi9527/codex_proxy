...
Code 内嵌管理保持适合窄标签页的 React 页面。一级管理员点击“打开完整管理平台”后，
合同入口 `/admin#browser?ticket=...&route=...` 会进入 `/admin/full`。该页面直接复用
standalone 的 `src/admin.html`、`src/admin_app.js` 和 `src/admin_modules/*`，因此
`TORVYE AI Gateway / 统一管理平台` 的概览、模型服务、中转节点、账号池、用量分析、
系统设置和教程只维护一套完整前端源码。中央模式的数据来自 Gateway 数据库与
Provider Worker，不读取或控制本机 `47892`；凭据只写不回显，完整控制台 API 只返回
脱敏预览，并由服务端强制 Level-1 和同源管理会话。

中央控制台的“检测/检测全部通道”会调用当前 Provider adapter 执行主动探测，不再把
历史健康状态伪装成本次检测结果。standalone adapter 使用上游 `OPTIONS` 或
`GET /models`；Provider Worker 模式下，ChatGPT 订阅通道通过真实额度刷新确认账号和
上游可达。当前 Worker 尚未承载的 Provider 会明确返回“不支持”，而不是显示误导性的
成功。中央用量页直接聚合 `usage_records`：按真实模型累计请求和 Token，并按
Asia/Shanghai 自然日生成最近 370 天的日历与趋势；没有调用的模型不会生成零请求记录。
...
- `gateway/admin-web/`：React/Vite 内嵌管理外壳、角色导航和普通用户账号页面；浏览器
  完整管理表面转入共享 TORVYE 控制台。
...
- `src/admin.html` / `src/admin_app.js` / `src/admin_modules/`：standalone 与中央
  Gateway 共享的完整 TORVYE 管理控制台、账号池、分析、设置和新手教程。
