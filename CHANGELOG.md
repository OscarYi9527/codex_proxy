...
- watchdog 恢复与源码部署彻底解耦：健康误判只允许校验并重启已安装版本，不再自动发布
  当前工作分支；连续失败阈值、监听 PID 归属、恢复后复检、冷却和日志轮转均已补齐。
- statistics 与 Provider 健康历史改为异步、串行、可合并的 5 秒批量持久化，失败采用
  指数退避并清理临时文件；每实体近期事件上限降为 1000，避免写盘阻塞 `/live`。
- 通过本地 HTTP(S) 代理的出站连接限制为最多 6 条、禁用流水线并缩短空闲保活，避免
  全账号检查、前台请求和重试同时压垮 `127.0.0.1:7890`。
- ChatGPT 单账号并发上限现在可按账号配置为 1～20，并可选择固定或自适应模式；客户端
  主动取消不再降低账号并发或继续轮换。运行中账号的状态检查会自动延后并显示为
  “正在处理请求”，不再用 usage 端点的并发故障覆盖真实路由状态。
- Responses 与 Chat Completions 的工具调用转换现在保留字符串参数和 custom tool 语义，
  并按工具索引处理并行流；流式 usage、incomplete/failed 终态和跨分包 SSE 解析已补齐。
- Provider、Gateway 与 Edge 流转发现在遵守下游背压，usage/回退错误缓存限制为 64 KiB；
  客户端断连后的结算 drain 设有 5 分钟上限，超大错误体会取消并安全进入回退。
