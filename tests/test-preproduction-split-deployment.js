...
describe('split preproduction deployment boundary', () => {
  ...
  it('locks activation and restores source, runtime image and service after failure', () => {
    ...
    assert.match(runner, /flock -x 8/)
    assert.match(runner, /nohup bash "\$\{BASH_SOURCE\[0\]\}"/)
  ...
  it('deploys Worker before Gateway and protects the shared standalone Proxy', () => {
    ...
    assert.match(deploymentTool, /\[IO\.File\]::Delete/)
    assert.doesNotMatch(deploymentTool, /Remove-Item/)
    assert.match(deploymentTool, /Guid\]::NewGuid/)
    assert.match(deploymentTool, /mkdir -p/)
    assert.match(deploymentTool, /chmod 700/)
    assert.match(deploymentTool, /DeploymentTimeoutMinutes/)
    assert.match(deploymentTool, /function Invoke-NativeProcessWithTimeout/)
    assert.match(deploymentTool, /function Invoke-DetachedReleaseStatus/)
    assert.match(deploymentTool, /function Invoke-SshWithRetry/)
    assert.match(deploymentTool, /function Invoke-ScpWithRetry/)
