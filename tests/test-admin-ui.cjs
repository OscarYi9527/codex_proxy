...
describe('admin error guide and login polling behavior', () => {
  ...
  it('offers synchronized quota refresh and a non-consuming all-account status check', () => {
    ...
    assert.match(appSource, /async function pollAccountCheckTask\(taskId\)/)
    assert.match(appSource, /chatgpt-accounts\/check-tasks/)
    assert.match(appSource, /async function cancelAccountCheckTask\(taskId\)/)
    assert.match(appSource, /async function resumeAccountCheckTask\(taskId\)/)
    assert.match(appSource, /可关闭窗口后继续在后台执行/)
    assert.match(appSource, /非消耗式状态检查/)
    assert.match(appSource, /当前套餐不支持用量端点/)
    assert.match(accountSource, /当前套餐不支持重置次数端点/)
    assert.match(accountSource, /连续失败/)
    assert.match(accountSource, /置信度/)
