...
- 单账号并发默认 3，可在账号策略中独立调整为 1～20；忙碌请求进入本地等待队列，单请求最多尝试 2 个账号，并优先使用 30 分钟内的新鲜额度数据。
- 每个账号可选择固定并发，或根据成功、429、网络错误和高延迟在 1～配置上限之间自适应；请求槽位使用可续期租约，断连或异常退出后可自动回收。
...
- 账号池提供不发送模型请求的“检查所有账号”后台任务，可查询进度、取消和断点恢复；逐号
  区分基础正常、额度不足、短时限流、登录失效、OAuth 不兼容、权限不足、暂时不可达及
  上游明确停用/封禁，并保留最近检查原因。
- 健康状态保存首次/最近发生时间、连续失败、置信度、探测范围和处置结果；网络/5xx 连续
  3 次才隔离，一次成功立即恢复，普通 `403` 只进入短期复核而不误判封号。
- 用量和重置次数分别使用 `synced/stale/unsupported/failed` 四态；不支持重置次数的套餐
  不再显示为 0 次或账号故障。
...
自启动会同时启动 watchdog。watchdog 只负责恢复**已经安装**的运行版本：

- 连续 5 次 `/live` 失败后才进入恢复判断；
- 端口仍由预期 Node 进程监听时，先延迟恢复，连续 3 轮仍卡死才重启；
- 每次恢复后同时复检 `/live`、监听 PID 和进程命令行，并设置 60 秒冷却；
- watchdog 不会调用部署脚本，也不会自动同步当前 Git 工作分支。

开发分支发布必须显式执行 `update-codex-proxy.ps1`。脚本会备份变化文件、
更新 SHA-256 manifest、优雅重启并验证运行目录；不要把健康恢复当成发布流程。
...
组织管理后端提供 `/api/v1/admin/organizations*`、`/api/v1/admin/accounts*` 和
`/api/v1/admin/invitations*`。Level-1 可管理全部组织、账号角色和一次性临时密码；
Level-2 的 repository 查询和写入被强制限制为本组织普通用户与邀请码，跨组织访问统一
返回 `403` 并写入脱敏管理审计。角色、组织、状态和到期变化会递增账号版本并撤销旧
Webview 管理会话；禁用、软删除、降级或间接禁用最后一个有效 Level-1 会在事务内以
`last_level1_protected` 拒绝。邀请码明文只在创建响应返回一次，数据库仅保存 keyed
digest，列表接口不会返回邀请码。

新建 Provider 凭据统一保存为 AES-256-GCM `envelope-v1`，AAD 绑定 Provider 与凭据 ID；
主密钥支持 Windows DPAPI、macOS Keychain、服务端环境 Keyring 或注入式 KMS adapter。
Gateway 启动时验证全部信封，Key 缺失或密文篡改会 fail-closed。遗留
`plaintext-v1` 仅允许回环 development/test 临时存在，production 和非回环部署仍拒绝
启动；可使用 `gateway:provider-credentials` 完成校验、事务迁移、轮换、重包和回滚。
完整离线操作流程见
[`docs/PROVIDER_CREDENTIAL_ENCRYPTION.md`](docs/PROVIDER_CREDENTIAL_ENCRYPTION.md)。

OpenAI 官方登录通过隔离 `CODEX_HOME` 启动 `codex app-server`，只向页面返回登录 URL 和
安全状态，完成后把 `auth.json` 加密导入 Gateway 数据库并删除临时目录，不读取或改写
共享 standalone 的账号文件。
...
业务操作。当前阶段尚未完成 T069+ 的积分风险预留与用量结算；不得把管理页面中的零值
...
| `POST` | `/admin/api/chatgpt-accounts/check-all` | 创建或返回当前非消耗式全账号检查任务（`202`） |
| `GET` | `/admin/api/chatgpt-accounts/check-tasks` | 查询最近账号检查任务 |
| `GET` | `/admin/api/chatgpt-accounts/check-tasks/:id` | 查询任务进度和逐号故障分类 |
| `POST` | `/admin/api/chatgpt-accounts/check-tasks/:id/cancel` | 协作式取消任务 |
| `POST` | `/admin/api/chatgpt-accounts/check-tasks/:id/resume` | 从最后提交批次恢复任务 |
| `GET` | `/admin/api/chatgpt-accounts/:id/health-events` | 查询指定账号最近的脱敏健康事件 |
...
| `CODEX_OUTBOUND_PROXY_CONNECTIONS` | `6` | 通过 `HTTP_PROXY` / `HTTPS_PROXY` 的最大并发连接数，限制为 `1–16` |
| `CODEX_ACCOUNT_CHECK_CONCURRENCY` | `2` | 后台全账号检查并发，限制为 `1–4` |
| `CODEX_ACCOUNT_CHECK_BATCH_SIZE` | `20` | 每次批量持久化的账号数，限制为 `1–100` |
| `CODEX_ACCOUNT_CHECK_TIMEOUT_MS` | `45000` | 单账号完整检查总超时，限制为 `1000–120000` |
| `CODEX_ACCOUNT_CHECK_JITTER_MIN_MS` / `MAX_MS` | `150` / `600` | 账号检查之间的随机抖动范围 |
...
- 非消耗式全账号后台检查、实时进度、关闭窗口继续执行、取消和重启断点恢复
...
| POST | /admin/api/chatgpt-accounts/check-all | 创建或返回当前非消耗式全账号检查任务 |
| GET | /admin/api/chatgpt-accounts/check-tasks | 查询最近检查任务 |
| GET | /admin/api/chatgpt-accounts/check-tasks/:id | 查询任务进度和逐号结果 |
| POST | /admin/api/chatgpt-accounts/check-tasks/:id/cancel | 取消任务 |
| POST | /admin/api/chatgpt-accounts/check-tasks/:id/resume | 恢复未完成任务 |
| GET | /admin/api/chatgpt-accounts/:id/health-events | 查询最近健康状态机事件 |
...
在账号卡片中打开“账号额度与预留策略”，可将“单账号最大并发”设置为 `1–20`，默认
为 `3`。账号卡片会同时显示活动请求数、当前有效并发和配置上限。

- **自适应并发（默认）**：配置值是恢复上限。遇到 `429`、网络错误、超时或超过
  120 秒的高延迟请求时逐级降低，最低为 `1`；每连续 8 次成功后恢复一级。
- **固定并发**：关闭“根据 429、网络错误和延迟自适应降并发”后，始终使用配置值。
  固定高并发不会绕过上游限制，设置过高可能增加 `429`、排队和连接失败。
- 客户端主动取消不计作账号故障，不会降低自适应并发，也不会继续轮换其他账号。

#### 全账号状态检查

“检查所有账号”会创建持久化后台任务，以最多 2 个并发账号逐号同步凭据、用量和重置
次数，不发送模型请求，也不会因此消耗模型额度。关闭进度窗口不会终止任务；窗口可显示
实时进度、取消当前任务，并从最后一个已提交批次恢复失败、中断或已取消任务。代理重启
后会自动恢复最近的中断任务。

正在处理模型请求的账号不会再并行发起状态探测：任务会自动重试 2 次，仍忙碌时显示
“正在处理请求”，不计为故障，也不会覆盖既有健康状态或误报“暂时不可达”。检查期间
即使一个短模型请求完整开始并结束，也会通过活动版本识别为竞态并延后结论。

任务按最多 20 个账号形成写入批次。批次内的 `checking`、Token 轮换、用量、重置次数和
最终健康结果通过 `JsonAccountStore` 合并，整份加密配置每批最多写入一次；并发发生的
账号改名或 Refresh Token 轮换不会被旧快照回退。任务状态和脱敏健康事件保存在独立运行
文件中，不包含 Access Token、Refresh Token 或 ID Token。结果会写入账号卡片，并可通过
“检查异常”筛选查看：
...
每次状态迁移记录 `first_seen_at`、`last_seen_at`、`consecutive_failures`、`confidence`、
`probe_scope`、`retry_at` 和 `disposition`。明确停用、永久凭据失效和 OAuth 不兼容会立即
停止调度；普通网络/`5xx` 只有连续 3 次失败才进入指数冷却隔离，一次成功检查会立即恢复。
普通 `403` 为中等置信度，只进入 15 分钟权限复核冷却，不标记为封号。

用量和重置次数独立持久化 `synced`、`stale`、`unsupported`、`failed`。用量按 30 分钟
判定陈旧，当前账号最低约 5 分钟刷新；重置次数独立约 6 小时刷新、12 小时判定陈旧。
重置次数端点返回 `404` 或明确“不支持”的 `403` 时标为 `unsupported`，保留最后一次
有效数据，不伪造 0 次，也不把账号计入故障。
...
| `codex-proxy.log.1` / `codex-proxy.error.log.1` | 上一次启动保留的标准输出与错误输出 |
...
统计和 Provider 健康历史使用异步、串行、可合并的批量写入。每个账号或
Provider 最多保留 1000 条近期事件，写盘失败采用指数退避并清理临时文件，
因此 `/live` 不会再因每次请求同步写入数 MB JSON 而长时间阻塞。
...
### watchdog 反复记录恢复，或 VS Code 使用一段时间后代理消失

先区分本地代理进程、出站桌面代理和上游服务：

```powershell
Invoke-RestMethod http://127.0.0.1:47892/live
Invoke-RestMethod http://127.0.0.1:47892/ready
Invoke-RestMethod http://127.0.0.1:47892/admin/api/runtime-info
Get-Content "$HOME\.codex-local-multi-proxy\codex-proxy-watchdog.log" -Tail 30
Get-Content "$HOME\.codex-local-multi-proxy\codex-proxy.error.log" -Tail 50
```

`runtime-info.consistency.status` 应为 `synchronized`。watchdog 不再自动部署源码；
出现 `installation_outdated` 时，应先完成测试，再显式运行更新脚本。

如果 `/live` 正常但错误日志持续出现 `ECONNRESET`、`CONNECT_TIMEOUT` 或
`ERR_SSL_WRONG_VERSION_NUMBER`，问题来自 `HTTP_PROXY` / `HTTPS_PROXY` 指向的
出站代理（例如 `127.0.0.1:7890`），不是 47892 watchdog 在终止它。当前版本默认把
该链路限制为 6 个连接并禁用流水线；资源较小的桌面代理可进一步设置：

```powershell
[Environment]::SetEnvironmentVariable(
  "CODEX_OUTBOUND_PROXY_CONNECTIONS", "3", "User"
)
```

随后重启 codex-proxy。不要通过降低 watchdog 阈值或连续重启来掩盖出站代理故障。
