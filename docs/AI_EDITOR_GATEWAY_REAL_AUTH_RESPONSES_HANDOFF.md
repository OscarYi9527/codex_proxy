...
npm run gateway:test                           45/45 passed
npm run admin:test                               1/1 passed
npm run test:coverage --workspace @ai-editor/gateway
                                               statements 90.64%, branches 73.44%
...
真实模式隔离启动复核确认 `47920/47921` 正常、初始状态为 `login_required`，Gateway
运行文件仅位于专用数据根；复核前后共享 `47892` PID 均为 `25592`。

以下不属于本轮 T023–T046 完成范围：

- T049+ 的可消费 Webview ticket、HttpOnly management session 和真实管理页面；
